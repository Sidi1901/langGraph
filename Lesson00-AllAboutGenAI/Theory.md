...
12. [LLM Inference](#llm-inference)
13. [Long Context vs CAG](#long-context-vs-cag)
14. [Prompt Caching](#prompt-caching)
15. [Memory](#memory)
...
If content is big enough, just skip the retrievl part right?

But it will be too costly.

Also, accuracy drops when context window content increases.

What if read the read the document once and then just remember? That's the CAG - Cache Augmented Generation

CAG works with KV cache. This is persisted in some memnory like disk memory. Inseeatd of re read docs, model just fetch KV.

CAG works best when source data don't change frequenctly othewise cache recomputing will be needed again and again. 
...
This can significanlty increase speed and reduce cost.
